...
def GetHIndexData(connection):
    query = "SELECT h.SongId, HIndex, Year FROM lyrics.HIndex2 h inner join lyrics.songs s on h.SongId = s.SongId;"
    df = pd.read_sql(query, connection)
    connection.close()
    return df
...
def UpdateStemmedLyrics(lyrics, songId, connection):
    with connection.cursor() as cursor:
           sql = "UPDATE songs SET StemmedLyrics = %s WHERE SongId = %s "
